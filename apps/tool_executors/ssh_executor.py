...
class SSHExecutor(BaseToolExecutor):
    ...
    def __init__(self, config: ToolConfig, connection_config: Dict[str, Any]):
        ...
        self.host = (_clean_secret(connection_config.get("host")) or "localhost")
        self.port = connection_config.get("port", 22)
        self.username = (_clean_secret(connection_config.get("username")) or "root")
        self.password = _clean_secret(connection_config.get("password"))
        self.key_file = _clean_secret(connection_config.get("key_file"))
        self.private_key_data = _clean_secret(connection_config.get("private_key"))
        self.passphrase = _clean_secret(connection_config.get("passphrase"))

        jump_host_cfg = connection_config.get("jump_host")
        if isinstance(jump_host_cfg, dict):
            self.jump_host = {
                "host": _clean_secret(jump_host_cfg.get("host")),
                "port": jump_host_cfg.get("port", 22),
                "username": _clean_secret(jump_host_cfg.get("username")),
                "password": _clean_secret(jump_host_cfg.get("password")),
                "key_file": _clean_secret(jump_host_cfg.get("key_file")),
                "private_key": _clean_secret(jump_host_cfg.get("private_key")),
                "passphrase": _clean_secret(jump_host_cfg.get("passphrase")),
            }
        else:
            self.jump_host = None

        self.sudo_password = _clean_secret(connection_config.get("sudo_password"))
        self.become_user = _clean_secret(connection_config.get("become_user"))
    ...
    async def _connect(self) -> None:
        ...
        for attempt in range(self._max_connection_attempts):
            try:
                ...
                if self.jump_host and self.jump_host.get("host"):
                    # Connect through jump host
                    jump_config = self.jump_host
                    logger.info(f"Connecting through jump host: {jump_config['host']}")

                    jump_client_keys = self._prepare_client_keys(
                        jump_config.get("key_file"),
                        jump_config.get("private_key"),
                        jump_config.get("passphrase"),
                    )
                    ...
                    client_keys = self._prepare_client_keys(
                        self.key_file, self.private_key_data, self.passphrase
                    )
    ...
    def _prepare_client_keys(self, key_file: Optional[str], private_key_data: Optional[str], passphrase: Optional[str]):
        if private_key_data:
            try:
                key = asyncssh.import_private_key(private_key_data, passphrase=passphrase)
                return [key]
            except Exception as exc:
                logger.error(f"Failed to import private key for {self.host}: {exc}")
        if key_file:
            return [key_file]
        return None
