    ...
    <div className="fixed inset-0 z-50 lg:hidden">
      ...
      <div className="fixed left-0 top-0 h-full w-80 bg-white shadow-xl">
        <div className="flex items-center justify-between p-4 border-b">
          <h2 className="text-lg font-semibold">Монитор задач</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {activeTasks.length === 0 ? (
            <div className="py-8 text-center text-gray-500">
              ...
              <p>Активные задачи отсутствуют</p>
            </div>
          ) : (
        ...
        {selectedTask && (
          <div className="border-t p-4">
            <h3 className="mb-3 font-medium">Детали задачи</h3>
            <div className="space-y-2 text-sm">
              <div>
                <span className="font-medium">ID:</span> {selectedTask.task_id}
              </div>
              <div>
                <span className="font-medium">Статус:</span> {selectedTask.status}
              </div>
              <div>
                <span className="font-medium">Шаг:</span> {selectedTask.current_step}
              </div>
