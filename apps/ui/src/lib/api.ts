...
export const environmentApi = {
  ...
  createEnvironment: async (payload: CreateEnvironmentRequest): Promise<Environment> => {
    const response = await api.post('/api/v1/environments', payload)
    return response.data
  },
  ...
  updateEnvironment: async (
    ...
    const response = await api.put(`/api/v1/environments/${environmentId}`, payload)
    return response.data
  },

  deleteEnvironment: async (environmentId: string): Promise<void> => {
    await api.delete(`/api/v1/environments/${environmentId}`)
  },
}
