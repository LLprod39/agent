...
async def test_agent(
    ...
    try:
        ...
        orchestration_request = OrchestrationRequest(
            ...
            metadata={"source": "admin_agent_test"},
        )
        ...
        return AgentTestResponse(
            test_id=test_id,
            status="success" if status_value == "completed" else status_value,
...
async def get_workflow_logs(
    ...
    try:
        ...
        async with db_manager.get_session() as session:
            ...
            task = await task_repo.get_by_id(task_id, include_steps=True)
            ...
            task_steps = await step_repo.get_by_task_id(task_id)
            audit_logs = await audit_repo.get_task_logs(task_id)

            workflow: Dict[str, Any] = {}
            for step in task_steps:
                step_key = step.step_id or str(step.id)
                workflow[step_key] = {
                    ...
                    "agent_type": step.agent_type.value if step.agent_type else None,
