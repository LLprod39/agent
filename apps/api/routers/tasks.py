...
async def create_task(
    ...
    task_step_repo: TaskStepRepository = Depends(get_task_step_repository),
    ...
    try:
        ...
        if request.environment_profile:
            if isinstance(request.environment_profile, dict):
                environment_data = dict(request.environment_profile)
                environment_id = environment_data.get("id")
            else:
                environment_id = request.environment_profile
                try:
                    profile = await environment_service.get_profile(environment_id)
                    if profile:
                        environment_data = dict(profile.data) if profile.data else {
                            "id": profile.id,
                            "type": profile.type,
                        ...
                        if environment_data is not None:
                            if profile.ssh and "ssh" not in environment_data:
                                environment_data["ssh"] = profile.ssh
                            elif (
                                isinstance(environment_data.get("extensions"), dict)
                                and "ssh" in environment_data["extensions"]
                                and "ssh" not in environment_data
                            ):
                                environment_data["ssh"] = environment_data["extensions"]["ssh"]
                            environment_data.setdefault("id", profile.id)
                except Exception as exc:  # pragma: no cover - defensive logging
                    logger.error(
                        "Failed to load environment profile %s: %s",
                        environment_id,
                        exc,
                    )

        orchestration_context = dict(request.context or {})
        if environment_data:
            orchestration_context.setdefault("environment", environment_data)
        elif environment_id:
            orchestration_context.setdefault("environment_id", environment_id)

        metadata_payload = {**(request.metadata or {}), **context}
        if environment_id:
            metadata_payload.setdefault("environment_profile_id", environment_id)

        # Prepare orchestration request
        orchestration_request = OrchestrationRequest(
            task=request.task,
            context=orchestration_context,
            environment_profile=environment_data,
            user_id=user_id,
        ...
        db_task = await task_repo.create(
            ...
            environment_profile=environment_id or (request.environment_profile if isinstance(request.environment_profile, str) else None),
            metadata={
        ...
        # Persist steps/results for workflow inspection
        if response.results:
            for index, result in enumerate(response.results, start=1):
                ...
                created_step = await task_step_repo.create_step(
                    task_id=str(db_task.id),
                    step_id=str(step_identifier),
                    description=description,
                    command=metadata.get("command"),
                    tool=metadata.get("tool") or result.get("agent"),
                    parameters=dict(metadata) if isinstance(metadata, dict) else {},
                ...
                if isinstance(raw_status, DBTaskStatus):
                    step_status = raw_status
                else:
                    status_value = getattr(raw_status, "value", raw_status)
                    try:
                        step_status = DBTaskStatus(status_value)
                    except (ValueError, TypeError):
                        step_status = (
                            DBTaskStatus.COMPLETED
                            if not result.get("error")
                            else DBTaskStatus.FAILED
                        )

                await task_step_repo.update_status(
                    str(created_step.id),
                    step_status,
                    output=(metadata.get("output") or result.get("output")),
